 ...
 * Absent optional sections are omitted from the JSON.
 */
...
public record Report(
     ...
     * @param findingsOmitted  findings left out per kind because of the per-kind cap
     */
    ...
    /** @param layer 0 = depends on no other directory; null for directories without Python files */
    @JsonInclude(JsonInclude.Include.ALWAYS)
    public record Component(String id, String path, int files, int lines, Integer layer) {
    }
    ...
    /**
     * @param kind     duplicate-module, repeated-logic, import-cycle, generated-file-committed,
     *                 missing-tests, unreferenced-file
     * @param severity warn or info
     */
    public record Finding(String id, String kind, String severity, String title, String detail,
                          List<Evidence> evidence) {
    }

    /** Where to look: a file, optionally a line range, and an optional note. */
    @JsonInclude(JsonInclude.Include.NON_NULL)
    public record Evidence(String path, Integer startLine, Integer endLine, String note) {
    }
